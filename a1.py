...
def login():
    if request.method == 'POST':
        ...
        conn = sqlite3.connect('C:/AS/database/attendance.db')
        cursor = conn.cursor()
...
def take_attendance():
    if request.method == 'POST':
        ...
        try:
            from face import FaceRecognizer  # Changed from f1 to face
            recognizer = FaceRecognizer()
            
            # Run recognition in the main thread (safer than threading for camera access)
...
def generate_report():
    ...
    filepath = os.path.join('C:/AS/reports', filename)
